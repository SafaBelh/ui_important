 ...
 */
import { buildWizardDataFromAnswers, PIPELINE_DEFS, TENANT_IDS_PLACEHOLDER, TABLE_PALETTE } from "@/constants/integrationWizard";
...
const STANDARD_FIELD_KEYS = new Set([
  "supplier", "supplierName", "amount", "date", "invoiceDate", "label",
  "status", "commandeRef", "commandeDate", "budgetCode", "category",
]);
...
export function preparePipelineTemplates(pipelines = {}) {
  return Object.fromEntries(Object.entries(pipelines).map(([key, pipeline]) => {
    ...
    const derivedExtras = userFields
      .filter(f => f.key && fm[f.key] && !STANDARD_FIELD_KEYS.has(f.key) && !extraAliases.has(f.key))
...
export const formatFieldLabel = (key = "") => String(key).replace(/([A-Z])/g, " $1").replace(/[_-]+/g, " ").replace(/^./, c => c.toUpperCase());
...
export function suggestJoinOn(base, joinTable, rels = []) {
  for (const r of (rels || [])) {
    if (r.from === base && r.to === joinTable) return `${base}.${r.col} = ${joinTable}.${r.toCol || r.col}`;
    if (r.from === joinTable && r.to === base) return `${base}.${r.toCol || r.col} = ${joinTable}.${r.col}`;
  }
  return null;
}
