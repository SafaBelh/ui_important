...
function JSONRender({ obj, depth = 0 }) {
  ...
  if (Array.isArray(obj)) {
    if (!obj.length)
      return (
        <span>
          <span className="asst-jbrace">[]</span>
        </span>
      );
    return (
      <span>
        ...
        {obj.map((v, i) => (
          <span key={i}>
            {pad1}
            <JSONRender obj={v} depth={depth + 1} />
            {i < obj.length - 1 ? "," : ""}
            {"\n"}
          </span>
        ))}
  ...
  return (
    <span>
      ...
      {entries.map(([k, v], i) => (
        <span key={k}>
          {pad1}
          <span className="asst-jkey">"{k}"</span>
...
function ReportView({ report, onAutofill, onBack: _onBack }) {
  ...
  const scoreTone =
    score >= 70 ? styles.scoreGood : score >= 40 ? styles.scoreWarn : styles.scoreBad;
  const handleCopy = () => {
    try {
    ...
    <div className="asst-report-wrap">
      <div className={cx(styles.reportScoreCard, scoreTone)}>
        <div className={styles.flexOne}>
          ...
          <progress
            className={cx(styles.scoreProgress, scoreTone)}
            value={score}
            max="100"
          ...
          <div className={styles.scoreHint}>
            {score >= 70
              ? "Configuration prête à être appliquée."
              : "Des champs importants sont manquants."}
          </div>
        </div>
        <button className="asst-pbtn" onClick={onAutofill}>
          <Wand2 size={15} /> Confirmer &amp; remplir le wizard
        </button>
      </div>
      <div className={styles.reportChipRow}>
        {[
          { label: report?.identity?.name || "Sans nom", tone: styles.chipRed },
          { label: report?.identity?.connectorType || "ERP", tone: styles.chipBlue },
          { label: report?.identity?.authType || "NONE", tone: styles.chipPurple },
          { label: `${(report?.tables?.selected || []).length} table(s)`, tone: styles.chipGreen },
          { label: `${(report?.tenants || []).length} tenant(s)`, tone: styles.chipAmber },
        ].map((c) => (
          <div key={c.label} className={cx(styles.reportChip, c.tone)}>
            {c.label}
          </div>
        ))}
      </div>
      <div className="asst-jblock">
        <div className="asst-jtbar">
          ...
          <button className="asst-cpbtn" onClick={handleCopy}>
            {copied ? <CheckCircle2 size={12} color="#4ade80" /> : <Copy size={12} />}{" "}
            {copied ? "Copié !" : "Copier"}
          </button>
          <button className="asst-cpbtn" onClick={handleDL}>
            <Download size={12} /> Télécharger
          </button>
        </div>
        <div className="asst-jcontent">
          <JSONRender obj={report} />
        </div>
      </div>
...
function JSONImportView({ onAutofill, onBack: _onBack }) {
  ...
  const handleConfirm = () => {
    ...
    const d = {
      ...
      tenants: (parsed.tenants || []).map((t) =>
        typeof t === "string"
          ? {
              ...
              active: false,
              statuses: {
                facture: { provisional: ["En attente"], final: ["Payé"], statusColumn: "STATUT" },
                commande: { provisional: ["En cours"], final: ["Livré"], statusColumn: "STATUT" },
              },
            }
          : t,
      ),
      pipelines: {
        facture: {
          enabled: parsed.pipelines?.factures?.enabled !== false,
          tables: parsed.pipelines?.factures?.sourceTables || [],
          fieldMappings: parsed.pipelines?.factures?.fieldMappings || {},
          conditions: [],
          joins: [],
          groupByCols: [],
        },
        commande: {
          enabled: parsed.pipelines?.commandes?.enabled !== false,
          tables: parsed.pipelines?.commandes?.sourceTables || [],
          fieldMappings: {},
          conditions: [],
          joins: [],
          groupByCols: parsed.pipelines?.commandes?.groupBy || [],
        },
      },
      budgetFormula: [],
      customPipelines: [],
      ...
      <div className={styles.jsonImportInner}>
        <div className={styles.mb20}>
          <div className={styles.jsonImportHeader}>
            <div>
              <div className={styles.jsonImportTitle}>Importer une configuration JSON</div>
              <div className={styles.jsonImportSubtitle}>
                Collez votre JSON de configuration ERP ci-dessous
              </div>
            </div>
            <button className={cx("asst-gbtn", styles.btnXs)} onClick={handleTemplate}>
        ...
        {score !== null && parsed && (
          <div
            className={cx(
              styles.importScoreCard,
              score >= 70 ? styles.scoreGood : styles.scoreWarn,
            )}
          >
            <div className={styles.importScoreHeader}>
              <div className={styles.importScoreTitle}>Score : {score}/100</div>
              {score >= 40 && (
                <button className="asst-pbtn" onClick={handleConfirm}>
                  <Wand2 size={14} /> Confirmer &amp; remplir le wizard
                </button>
              )}
            </div>
            <progress
    ...
    <div className="asst-chip-row">
      {options.map((opt) => {
        ...
        return (
          <button
            key={opt.value}
            className={`asst-chip${s ? " sel" : ""}`}
            onClick={() =>
              onChange(s ? value.filter((x) => x !== opt.value) : [...value, opt.value])
            }
          >
            {opt.label}
          </button>
...
const SMART_FORM_SECTIONS = [
  {
    ...
    fields: [
      ...
      },
      {
        ...
        type: "select",
        options: ["ERP", "DATA_SOURCE", "ACCOUNTING"],
      },
      {
        key: "authType",
        label: "Authentification",
        type: "select",
        options: ["NONE", "BASIC", "API_KEY", "OAUTH2", "JWT_SIGNED", "SAML"],
      },
      {
        key: "description",
        label: "Description",
  ...
  {
    id: "auth_details",
    label: "Détails d'authentification",
    color: "#6366f1",
    Icon: Settings2,
    fields: [],
    dynamic: true,
...
function SmartFormPanel({ formData, setFormData, schema, onSubmit }) {
  ...
  const renderF = (f) => {
    ...
    if (f.type === "table_picker") {
      ...
      return (
        <div key={f.key} className={styles.spanAll}>
          <label className="asst-flabel">{f.label}</label>
          {allTables.length === 0 ? (
            <div className={styles.emptyHint}>Saisissez d'abord une URL JDBC</div>
          ) : (
            <div className="asst-chip-row">
              {allTables.map((t) => {
                const s = sel.includes(t.name);
                return (
                  <button
                    key={t.name}
                    className={`asst-chip${s ? " sel" : ""}`}
                    onClick={() =>
                      update(f.key, s ? sel.filter((x) => x !== t.name) : [...sel, t.name])
                    }
                  >
                    {t.name}
                  </button>
    ...
    if (f.type === "table_subset" || f.type === "table_subset_key") {
      ...
      return (
        <div key={f.key} className={styles.spanAll}>
          <label className="asst-flabel">{f.label}</label>
          {avail.length === 0 ? (
            <div className={styles.emptyHint}>Sélectionnez d'abord des tables</div>
          ) : (
            <div className="asst-chip-row">
              {avail.map((t) => {
                const s = v2.includes(t.name);
                return (
                  <button
                    key={t.name}
                    className={`asst-chip${s ? " sel" : ""}`}
                    onClick={() =>
                      update(f.key, s ? v2.filter((x) => x !== t.name) : [...v2, t.name])
                    }
                  >
                    {t.name}
                  </button>
    ...
    if (f.type === "select")
      return (
        <div key={f.key} className={spanAll}>
          <label className="asst-flabel">{f.label}</label>
          <select
            className="asst-fsel"
            value={v || ""}
      ...
      return (
        <div key={f.key}>
          ...
          <div className={styles.colorRow}>
            ...
            />
            <input
              className={cx("asst-fi", styles.colorTextInput)}
              value={v || "#D94F3D"}
              onChange={(e) => update(f.key, e.target.value)}
            />
          </div>
      ...
      return (
        <div key={f.key} className={spanAll}>
          ...
          <div className={styles.passwordWrap}>
            <input
              type={pwVis[f.key] ? "text" : "password"}
              className={cx("asst-fi", styles.passwordInput)}
              value={v || ""}
              onChange={(e) => update(f.key, e.target.value)}
              placeholder={f.placeholder}
            />
            <button
              onClick={() => setPwVis((p) => ({ ...p, [f.key]: !p[f.key] }))}
              className={styles.passwordToggle}
            >
              {pwVis[f.key] ? <EyeOff size={14} /> : <Eye size={14} />}
            </button>
          </div>
        </div>
      );
    return (
      <div key={f.key} className={spanAll}>
        ...
        <input
          ...
          className={cx("asst-fi", f.mono && styles.monoInput)}
          value={v || ""}
          onChange={(e) => update(f.key, e.target.value)}
          placeholder={f.placeholder || ""}
          maxLength={f.maxLen}
    ...
    <div className="asst-form-wrap">
      <div className={styles.formProgressRow}>
        <progress
          ...
          value={pct}
          max="100"
          aria-label={`Formulaire rempli à ${pct}%`}
        />
      ...
      {SMART_FORM_SECTIONS.map((sec) => {
        ...
        return (
          <div key={sec.id} className="asst-fsec">
            <div className="asst-fsec-hdr">
              <div className={cx(styles.sectionIcon, styles[`sectionIcon_${sec.id}`])}>
                <SI size={15} color={sec.color} />
              </div>
              <div className={styles.sectionTitle}>{sec.label}</div>
            </div>
            <div className="asst-fsec-body">
              <div className="asst-frow">{fields.map((f) => renderF(f))}</div>
            </div>
          </div>
        );
      })}
      <div className={styles.formFooter}>
        <button className="asst-pbtn" onClick={onSubmit}>
          <Sparkles size={15} /> Générer le rapport JSON
        </button>
      </div>
    </div>
  );
...
export function AssistantFullscreen({ onClose, onAutofill, rawSchema, initialMode, onMinimize }) {
  ...
  const schema = useMemo(() => {
    ...
    if (rawSchema) {
      ...
      return {
        tables,
        rels: rawSchema.rels.filter((r) => tableNames.has(r.from) && tableNames.has(r.to)),
      };
    }
  ...
  useEffect(() => {
    ...
    setTimeout(
      () =>
        setMessages([
          { role: "bot", text: firstStep.bot, type: firstStep.type, options: firstStep.options },
        ]),
      80,
    );
  }, [messages.length]);
  ...
  const initChat = () => {
    ...
    const s = QA_FLOW[0];
    setTimeout(
      () => setMessages([{ role: "bot", text: s.bot, type: s.type, options: s.options }]),
      80,
    );
  };
  const advanceToStep = useCallback(
    (targetId, curAnswers) => {
      ...
      setTimeout(() => {
        ...
        if (ns.type === "choice" || ns.type === "mode_pick") opts.options = ns.options;
        else if (ns.type === "multi_schema")
          opts.schOpts = (schema?.tables || GENERIC_SCHEMA.tables).map((t) => ({
            label: t.name,
            value: t.name,
          }));
        else if (ns.type === "choice_dynamic")
          opts.dynOpts = (curAnswers.selectedTables || []).map((n) => ({ label: n, value: n }));
          ...
          const plCols = (schema?.tables || GENERIC_SCHEMA.tables)
            .filter((t) => selTables.includes(t.name))
            .flatMap((t) =>
              t.cols.map((c) => ({ label: `${t.name}.${c}`, value: `${t.name}.${c}` })),
            );
          opts.schOpts = plCols;
  ...
  const handleModeChoice = (v) => {
    ...
    if (v === "qa") {
      ...
      setTimeout(() => {
        ...
        addBot(
          v === "form"
            ? "Parfait ! Remplissez le formulaire ci-dessous."
            : "Collez votre JSON de configuration ci-dessous.",
          { type: "switch_" + v },
        );
      }, 400);
    }
    ...
    <div className="asst-fs">
      <div className="asst-fs-hdr">
        <div className={styles.fsTitleRow}>
          <div className={styles.fsBotIcon}>
            <Bot size={18} color="#D94F3D" />
          </div>
          <div>
            <div className={styles.fsTitle}>Assistant Configuration ERP</div>
            <div className={styles.fsSubtitle}>
        ...
        <div className={styles.fsActions}>
          ...
          {mode && !showReport && !isJSON && !initialMode && (
            <div className="mode-tabs">
              {[
                { v: "qa", I: MessageSquare, label: "Q&A" },
                { v: "form", I: FileJson, label: "Formulaire" },
                { v: "json", I: Upload, label: "JSON" },
              ].map(({ v, I, label }) => (
                <button
                  key={v}
          ...
          {(showReport || isJSON) && (
            <button className="asst-gbtn" onClick={() => setShowReport(false)}>
              <ArrowLeft size={13} /> Retour
            </button>
          )}
          <button onClick={initChat} title="Recommencer" className={styles.iconButton}>
            <RotateCcw size={14} />
          </button>
          {onMinimize && (
            <button onClick={onMinimize} title="Réduire" className={styles.iconButton}>
              <Minimize2 size={14} />
            </button>
          )}
          <button onClick={onClose} title="Fermer" className={styles.iconButton}>
            <X size={15} />
          </button>
        </div>
      </div>
      {mode === "qa" && (
        <progress
          className={cx("asst-prog-rail", styles.qaProgress)}
          value={modePct}
          max="100"
          aria-label={`Progression ${modePct}%`}
        />
      )}
      <div className="asst-fs-body">
        {showSidebar && (
          <div className="asst-fs-sidebar">
            <div className={styles.sidebarTitle}>Étapes</div>
            {QA_SIDEBAR_STEPS.map((s, i) => {
              const idx = QA_FLOW.findIndex((q) => q.id === s.id);
              const done = qaStepIdx > idx,
                active = curStep?.id === s.id;
              return (
                <div key={s.id} className={`asst-ss${active ? " active" : done ? " done" : ""}`}>
                  <div className="asst-sn">
                    {done ? <CheckCircle2 size={11} color={INTEGRATION_COLORS.success} /> : i + 1}
                  </div>
                  <span
                    className={cx(
                      styles.sidebarStepLabel,
                      active && styles.sidebarStepActive,
                      done && styles.sidebarStepDone,
                    )}
                  >
                    {s.label}
                  </span>
        ...
        <div className="asst-fs-main">
          {showReport && report ? (
            <ReportView
              report={report}
              onAutofill={handleAutofillAndClose}
              ...
              <div className="asst-chat-wrap">
                <div className="asst-chat-inner">
                  {messages.map((msg, i) => (
                    <div
                      key={i}
                      className={`${msg.role === "bot" ? "asst-msg-bot" : "asst-msg-user"} asst-anim`}
                    >
                      {msg.role === "bot" && (
                      ...
                      <div className={styles.chatMsgContent}>
                            ...
                            <div className={styles.mt10}>
                              {(msg.options || []).map((opt, oi) => (
                                <button
                                  key={oi}
                                  className="asst-opt"
                                  onClick={() => handleModeChoice(opt.value)}
                                >
                                  <span className={styles.flexOne}>{opt.label}</span>
                                  {opt.desc && (
                                    <span className={styles.optionDesc}>{opt.desc}</span>
                                  )}
                                  <ChevronRight size={14} color={INTEGRATION_COLORS.g300} />
                                </button>
                              ))}
                            </div>
                          )}
                            ...
                            <div className={styles.mt10}>
                              {(msg.options || []).map((opt, oi) => (
                                <button
                                  key={oi}
                                  className="asst-opt"
                                  onClick={() => advanceQA(curStep?.key, opt.value, opt.label)}
                                >
                                  <span className={styles.flexOne}>{opt.label}</span>
                                  <ChevronRight size={14} color={INTEGRATION_COLORS.g300} />
                                </button>
                        ...
                        {msg.role === "bot" &&
                          msg.type === "choice_dynamic" &&
                          i === messages.length - 1 && (
                            <div className={styles.mt10}>
                              {(msg.dynOpts || []).length === 0 ? (
                                <div className={styles.emptyHint}>Aucune table sélectionnée</div>
                              ) : (
                                (msg.dynOpts || []).map((opt, oi) => (
                                  <button
                                    ...
                                    onClick={() => advanceQA(curStep?.key, [opt.value], opt.label)}
                                  >
                                    <Database size={13} color={INTEGRATION_COLORS.g400} />
                                    <span className={styles.flexOne}>{opt.label}</span>
                                    <ChevronRight size={14} color={INTEGRATION_COLORS.g300} />
                                  </button>
                                ))
                        ...
                        {msg.role === "bot" &&
                          ...
                          i === messages.length - 1 && (
                            <div className={styles.mt10}>
                              ...
                              />
                              <button
                                className={cx("asst-pbtn", styles.inlineConfirm)}
                                disabled={multiSel.length === 0}
                                onClick={() =>
                                  advanceQA(curStep?.key, multiSel, `${multiSel.length} table(s)`)
                                }
                              >
                        ...
                        {msg.role === "bot" &&
                          ...
                          i === messages.length - 1 && (
                            <div className={styles.mt10}>
                              ...
                              <button
                                className={cx("asst-gbtn", styles.inlineSkip)}
                                onClick={() => advanceQA(curStep?.key, {}, "Mapping ignoré")}
                              >
                                Passer
                              </button>
                            </div>
                          ...
                          i === messages.length - 1 && (
                            <div className={styles.mt10}>
                              ...
                              <div className={styles.inlineButtonRow}>
                                <button
                                  className={cx("asst-pbtn", styles.inlineButton)}
                                  onClick={() =>
                                    advanceQA(
                                      curStep?.key,
                                      multiSel,
                                      multiSel.length
                                        ? `${multiSel.length} col(s)`
                                        : "Pas de Group By",
                                    )
                                  }
                                >
                                ...
                                <button
                                  className={cx("asst-gbtn", styles.inlineSkipCompact)}
                        ...
                        {msg.role === "bot" && msg.type === "done" && (
                          <button
                            className={cx("asst-pbtn", styles.mt12)}
                            onClick={() => genReport(answers)}
                          >
                  ...
                  {typing && (
                    <div className="asst-msg-bot asst-anim">
                      <div className={styles.chatBotIconNoMargin}>
                        <Bot size={15} color="#D94F3D" />
                      </div>
        ...
        <div className={styles.validationItems}>
          {shown.map((it, i) => (
            <div key={i} className={cx(styles.validationItem, tone)}>
              ...
              <span className={styles.validationSep}> · </span>
              {it.message}
            </div>
          ))}
          {items.length > shown.length && (
            <div className={styles.validationMore}>+{items.length - shown.length} autre(s)…</div>
          )}
        </div>
      </div>
    );
    ...
    <div>
      <div
        className={cx(
          ...
          result.ok ? styles.validationOk : styles.validationError,
        )}
      >
...
export function AssistantMiniChat({
  onClose,
  onAutofill,
  onOpenFullscreen,
  knownPlatformTenantIds,
}) {
  ...
  const say = (role, text, extra = {}) => {
    if (mounted.current) setMessages((p) => [...p, { role, text, ...extra }]);
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      say(
        "bot",
        "Bonjour, je suis l'assistant ERP d'AnomalyIQ. Je vous aide à configurer un nouveau connecteur, étape par étape.",
      );
      await sleep(700);
      if (cancelled || !mounted.current) return;
  ...
  const chooseJson = async () => {
    ...
    say(
      "bot",
      "Téléchargez le modèle, remplissez-le avec les détails de votre ERP, puis importez le fichier.",
      { actions: "download" },
    );
  };
  ...
  const downloadTemplate = async () => {
    ...
    a.href = url;
    a.download = "erp-connector-template.json";
    ...
    say(
      "bot",
      "J'attends votre fichier rempli. Importez-le quand il est prêt — je le vérifie automatiquement.",
      { actions: "import" },
    );
  };
  ...
  const onPickFile = async (e) => {
    ...
    try {
      ...
      say("bot", "Impossible de lire le fichier.", { error: true, actions: "import" });
      return;
    }
  ...
  const handleImport = async (text) => {
    ...
    say(
      "bot",
      "Je vérifie votre configuration : champs requis, identifiants SQL, mappings, budget…",
    );
    await sleep(650);
    ...
    if (!res.ok && res.stage === "parse") {
      ...
      say("bot", "Le fichier n'est pas un JSON valide. Corrigez-le puis réimportez-le.", {
        actions: "import",
      });
      setStage("await-import");
      return;
    ...
    if (!res.ok) {
      say(
        "bot",
        "Il manque des informations ou certaines sont invalides (voir ci-dessus). Corrigez le fichier, puis réimportez-le.",
        { actions: "import" },
      );
      setStage("await-import");
      return;
    ...
    say(
      "bot",
      `Configuration valide et complète — « ${sum.name} » · ${sum.connectionType.toUpperCase()} · ${sum.tables} table(s) · ${sum.pipelines.length} pipeline(s)${pw}.`,
      { icon: CheckCircle2, iconColor: INTEGRATION_COLORS.success },
    );
    await sleep(1300);
    if (!mounted.current) return;
    say("bot", "Je crée le connecteur et j'ouvre le formulaire pré-rempli pour vérification…", {
      icon: Wand2,
      iconColor: INTEGRATION_COLORS.red,
    });
    await sleep(1100);
    if (!mounted.current) return;
    ...
    if (kind === "choice")
      return (
        <div className={styles.miniChoiceActions}>
          <button onClick={chooseJson} className={cx("btn btn-primary", styles.miniChoiceButton)}>
            <FileJson size={14} /> Import JSON
          </button>
          <button
            onClick={chooseQA}
            className={cx("btn btn-ghost", styles.miniChoiceButton, styles.miniGhostButton)}
          >
            <MessageSquare size={14} /> Questions / Réponses
          </button>
        </div>
      );
    if (kind === "download")
      return (
        <button onClick={downloadTemplate} className={cx("btn btn-primary", styles.miniFullAction)}>
    ...
    if (kind === "import")
      return (
        <button
          ...
          className={cx("btn btn-primary", styles.miniFullAction)}
        >
          <Upload size={14} /> Importer le fichier JSON
        </button>
      );
    return null;
  };
  ...
  return createPortal(
    <div className={styles.miniChat}>
      ...
      <div className={styles.miniHeader}>
        <div className={styles.miniHeaderIcon}>
          <Bot size={18} color="#fff" />
        </div>
        <div className={styles.miniHeaderText}>
          <div className={styles.miniTitle}>Assistant ERP</div>
          <div className={styles.miniSubtitle}>
            <span className={styles.miniStatusDot} /> Configuration guidée · 100% local
          </div>
        </div>
        <button onClick={onClose} title="Fermer" className={styles.miniClose}>
          <X size={15} />
        </button>
      </div>
          ...
          return (
            <div
              ...
              className={cx(
                ...
                isBot ? styles.miniMessageBotRow : styles.miniMessageUserRow,
              )}
            >
              {isBot && (
                <div className={styles.miniAvatar}>
                  <Bot size={13} color={INTEGRATION_COLORS.red} />
                </div>
              )}
              <div
                className={cx(
                  styles.miniBubble,
                  isBot ? styles.miniBubbleBot : styles.miniBubbleUser,
                  isBot && m.error && styles.miniBubbleError,
                  m.report && styles.miniBubbleReport,
                )}
              >
                {m.report ? (
                  <ValidationReport result={m.report} />
                ) : (
                  <span className={styles.miniTextRow}>
                    {Ic && (
                      <Ic
                        size={14}
                        color={m.iconColor || (isBot ? INTEGRATION_COLORS.g500 : "#fff")}
                        className={styles.miniInlineIcon}
                      />
                    )}
                    <span>{m.text}</span>
                  </span>
