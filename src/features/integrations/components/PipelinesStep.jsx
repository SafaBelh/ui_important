...
import {
  ...
  SectionAccordion,
  VisualJoinBuilder,
} from "@/features/integrations/components/WizardUiPrimitives";
...
export function PipelinesStep({ data, setData, schema }) {
  ...
  const addCustomPipeline = () => {
    ...
    setData({
      ...
      customPipelines: [
        ...customPipelines,
        { id, label: newPipelineName.trim(), color: CUSTOM_PIPELINE_COLORS[colorIdx] },
      ],
    });
    setNewPipelineName("");
  ...
  const removeCustomPipeline = (id) => {
    ...
    delete nextPipelines[id];
    setData({ ...data, customPipelines: next, pipelines: nextPipelines });
    setActiveTab("facture");
  };
  const setPipeline = useCallback(
    (k, v) => setData({ ...data, pipelines: { ...pipelines, [k]: v } }),
    [data, pipelines, setData],
  );
  ...
  const toggleTable = (tname) =>
    setPl({
      ...pl,
      tables: pl.tables.includes(tname)
        ? pl.tables.filter((t) => t !== tname)
        : [...pl.tables, tname],
    });
  const plTables = activeTables.filter((t) => (pl.tables || []).includes(t.name));
  const plCols = plTables.flatMap((t) =>
    t.cols.map((c) => ({ full: `${t.name}.${c}`, table: t.name, col: c })),
  );
  const fixedFields = (
    builtinDef?.fixedFields ||
    (!isBuiltin
      ? [
  ...
  const mkOption = (f, source) => ({
    key: f.key,
    label: (f.label || f.key).replace(/^Groupe:\s*/, ""),
    source,
    mapped: !!(gbFm[f.key] && String(gbFm[f.key]).trim()),
  });
    ...
    ? activeTab === "facture"
      ? [
          ...fixedFields
            ...
            .map((f) => mkOption(f, "Champ standard")),
  ...
  const toggleGroupBy = (key, mapped) => {
    ...
    setPl({
      ...pl,
      groupByCols: current.includes(key)
  ...
  const addGroupByFromColumn = (full) => {
    ...
    if (known) {
      setPl({
        ...
        fieldMappings: {
          ...(pl.fieldMappings || {}),
          [colName]: (pl.fieldMappings || {})[colName] || full,
        },
    ...
    setPl({
      ...
      userFields: [
        ...userFields,
        { id: `gb_${Date.now()}`, key: colName, label: colName, type: "text", required: false },
      ],
  ...
  const activeUnmappedGb = activeGbCols.filter(
    (c) => !(pl.fieldMappings || {})[c] || !String((pl.fieldMappings || {})[c]).trim(),
  );
  const showGroupByEmptyError =
    requiresGroupBy && pl.enabled !== false && activeGbCols.length === 0;
  const showGroupByError =
    pl.enabled !== false && (showGroupByEmptyError || activeUnmappedGb.length > 0);
  ...
  useEffect(() => {
    ...
    const next =
      withoutSupplier.length > 0 ? withoutSupplier : PIPELINE_DEFS.commande.defaultGroupByCols;
  ...
  return (
    <div className={styles.pipelineRoot} data-theme={pipelineTheme}>
      <InfoBox color={INTEGRATION_COLORS.info}>
        Configurez les pipelines métier — tables, jointures, mapping.
      </InfoBox>
      {/* Tab bar */}
      <div className={styles.tabBar}>
        {["facture", "commande"].map((key) => {
          ...
          return (
            <button
              ...
              data-theme={getPipelineTheme(def.color)}
        ...
        {customPipelines.map((cp) => (
          <div
            ...
            data-theme={getPipelineTheme(cp.color)}
            data-selected={activeTab === cp.id}
          >
            <button
              onClick={() => setActiveTab(cp.id)}
              className={cx(styles.tabButton, styles.customTabMain)}
            >
              <Settings2 size={13} /> {cp.label}
            </button>
            <button onClick={() => removeCustomPipeline(cp.id)} className={styles.customTabRemove}>
              <X size={11} />
            </button>
          </div>
        ))}
        {!showAddPipeline ? (
          <button onClick={() => setShowAddPipeline(true)} className={styles.addPipelineButton}>
            <Plus size={12} /> Nouveau pipeline
          </button>
        ) : (
          <div className={styles.addPipelineForm}>
            <input
              value={newPipelineName}
              onChange={(e) => setNewPipelineName(e.target.value)}
              onKeyDown={(e) => e.key === "Enter" && addCustomPipeline()}
              autoFocus
              className={cx("input", styles.pipelineNameInput)}
              placeholder="Nom du pipeline…"
            />
            <button
              className={cx("btn btn-primary", styles.createPipelineButton)}
              onClick={addCustomPipeline}
              disabled={!newPipelineName.trim()}
            >
              Créer
            </button>
            <button onClick={() => setShowAddPipeline(false)} className={styles.iconButtonMuted}>
              <X size={13} />
            </button>
          </div>
        )}
      ...
      <div className={styles.pipelineHeader}>
        <div className={styles.pipelineIcon}>
          <PlIcon size={16} color={plColor} />
        </div>
        <div className={styles.grow}>
          <div className={styles.pipelineTitle}>Pipeline {plLabel}</div>
        </div>
      ...
      {pl.enabled !== false && (
        <>
            ...
            <div className={styles.tableList}>
              {activeTables.length === 0 ? (
                <div className={styles.emptyTables}>
                  Sélectionnez des tables à l'étape Connexion
                </div>
              ) : (
                activeTables.map((t) => {
                  const sel = pl.tables.includes(t.name);
                  return (
                    <div
                      key={t.name}
                    ...
                    >
                      ...
                      <span className={cx("mono", styles.tableName)}>{t.name}</span>
                    </div>
                  );
                })
              )}
          ...
          {pl.tables.length > 1 && (
            <SectionAccordion
              icon={<Link2 size={13} color={INTEGRATION_COLORS.warning} />}
              title="Jointures (JOIN)"
              subtitle={`${pl.tables.length} tables — configurez les conditions ON`}
            >
              <VisualJoinBuilder
                tables={pl.tables}
                joins={pl.joins || []}
          ...
          <SectionAccordion
            icon={<GitBranch size={13} color={plColor} />}
            ...
            <div className={styles.sectionStack}>
              <div className={cx(styles.mappingRow, styles.tenantRow)}>
                <div className={styles.mappingLabelBlock}>
                  ...
                  <div className={styles.mappingHint}>
                    multi-tenant : filtre l'import par l'external id
                  </div>
                </div>
                <span className={styles.arrow}>→</span>
                <select
                  value={pl.tenantColumn || ""}
                  onChange={(e) => setPl({ ...pl, tenantColumn: e.target.value })}
                  className={cx("select", styles.flexSelect)}
                >
              ...
              <div className={cx(styles.mappingRow, styles.sourceKeyRow)}>
                <div className={styles.mappingLabelBlock}>
                  ...
                  <div className={styles.mappingHint}>
                    PK stable (ex: facture_id) pour le curseur de polling
                  </div>
                </div>
                <span className={styles.arrow}>→</span>
                <select
                  value={pl.sourceKeyColumn || ""}
                  onChange={(e) => setPl({ ...pl, sourceKeyColumn: e.target.value })}
                  className={cx("select", styles.flexSelect)}
                >
                ...
                <div className={styles.fieldStack}>
                  {fixedFields.map((f) => (
                    <div key={f.key} className={styles.mappingRow}>
                      <div className={styles.mappingLabelBlock}>
                        <div className={styles.mappingTitle}>{f.label}</div>
                        {f.required && <div className={styles.requiredText}>Requis</div>}
                      </div>
                      <span className={styles.arrow}>→</span>
                      <select
                        value={(pl.fieldMappings || {})[f.key] || ""}
                        onChange={(e) =>
                          setPl({
                            ...pl,
                            fieldMappings: { ...(pl.fieldMappings || {}), [f.key]: e.target.value },
                          })
                        }
                        className={cx("select", styles.flexSelect)}
                      >
              ...
              {(fixedFields.length === 0 || builtinDef?.allowExtraFields || !isBuiltin) && (
                <div className={styles.fieldStack}>
                  <div className={styles.extraFieldHeader}>
                    <div className={styles.grow}>
                      <div className={styles.extraFieldTitle}>
                        {!isBuiltin ? "Champs de regroupement" : "Champs additionnels"}
                      </div>
                      <div className={styles.extraFieldHint}>
                        {!isBuiltin
                          ? "Ajoutez uniquement les dimensions qui définissent une série. Le moteur analyse date + montant dans chaque groupe."
                          : "Définissez des champs additionnels puis associez-les aux colonnes sources."}
                      </div>
                    </div>
                    <button
                      type="button"
                      onClick={addUserField}
                      className={cx("btn btn-ghost", styles.addFieldButton)}
                    >
                      <Plus size={12} /> Ajouter un champ
                    </button>
                  </div>
                  {userFields.length === 0 ? (
                    <div className={styles.emptyCustomFields}>
                      Aucun champ personnalisé. Ajoutez un champ pour configurer un nouveau pipeline
                      métier.
                    </div>
                  ) : (
                    userFields.map((field) => (
                      <div key={field.id} className={styles.userFieldRow}>
                        <input
                          value={field.key}
                          onChange={(e) => updateUserField(field.id, "key", e.target.value.trim())}
                          className={cx("input mono", styles.userFieldKey)}
                          placeholder="cle_champ"
                        />
                        <input
                          value={field.label}
                          onChange={(e) => updateUserField(field.id, "label", e.target.value)}
                          className={cx("input", styles.userFieldLabel)}
                          placeholder="Libellé"
                        />
                        <select
                          value={field.type || "text"}
                          onChange={(e) => updateUserField(field.id, "type", e.target.value)}
                          className={cx("select", styles.userFieldType)}
                        >
                        ...
                        <label className={styles.requiredToggle}>
                          <input
                            ...
                            onChange={(e) =>
                              updateUserField(field.id, "required", e.target.checked)
                            }
                          />{" "}
                          Requis
                        </label>
                        <select
                          value={(pl.fieldMappings || {})[field.key] || ""}
                          onChange={(e) => setUserFieldMapping(field, e.target.value)}
                          disabled={!field.key}
                          className={cx("select", styles.userFieldSource)}
                        >
          ...
          {supportsGroupByOverride && (
            <SectionAccordion
              icon={<Layers size={13} color={plColor} />}
              title="Regroupement des séries"
              subtitle={
                activeTab === "facture"
                  ? "Par défaut: fournisseur + label. Une sélection explicite remplace ce défaut."
                  : "Obligatoire pour commandes et pipelines personnalisés"
              }
            >
              <div className={styles.sectionStack}>
                <InfoBox
                  color={showGroupByError ? INTEGRATION_COLORS.red : INTEGRATION_COLORS.info}
                >
                  {activeTab === "facture"
                ...
                {groupByOptions.length === 0 ? (
                  <div className={styles.emptyGroupBy}>
                    Aucun champ disponible pour le regroupement. Ajoutez des champs personnalisés
                    pour ce pipeline.
                  </div>
                ) : (
                    ...
                    {groupByOptions.map((option) => {
                      ...
                      return (
                        <label
                          ...
                          title={
                            disabled
                              ? "Mappez ce champ à une colonne source dans « Mapping des champs » avant de l'utiliser"
                              : ""
                          }
                        ...
                        >
                          <input
                            ...
                            checked={checked}
                            disabled={disabled}
                            onChange={() => toggleGroupBy(option.key, option.mapped)}
                          />
                          <span className={styles.groupByContent}>
                            <span className={styles.groupByLabel}>
                              {option.label}
                              {disabled && <span className={styles.mapBadge}>à mapper</span>}
                            </span>
                            <span className={cx("mono", styles.groupByMeta)}>
                              {option.key} · {option.source}
                            </span>
                          </span>
                        </label>
                      );
                ...
                <div className={styles.groupBySourceRow}>
                  <select
                    className={cx("select", styles.groupBySourceSelect)}
                    value=""
                    disabled={plCols.length === 0}
                    onChange={(e) => {
                      addGroupByFromColumn(e.target.value);
                      e.target.value = "";
                    }}
                  >
                    <option value="">
                      + Regrouper aussi par une colonne source (ex: centre_code)…
                    </option>
                    {plCols.map((c) => (
                      <option key={c.full} value={c.full}>
                        {c.full}
                  ...
                  <span className={styles.groupBySourceHint}>
                    Utile quand un même fournisseur émarge sur plusieurs centres budgétaires :
                    combinez fournisseur + label + centre pour des séries distinctes.
                  </span>
                </div>
                {showGroupByError && (
                  <div className={styles.groupByError}>
                    <AlertCircle size={14} />{" "}
                    {activeUnmappedGb.length > 0
          ...
          <SectionAccordion
            icon={<Clock size={13} color={plColor} />}
            title="Rythme d'exécution"
            subtitle="Appliqué automatiquement aux pipelines créés à l'activation des tenants"
          >
            <div className={styles.sectionStack}>
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        setPl({ ...pl, schedule: { ...(pl.schedule || {}), mode: m.id } })
                      }
              ...
              {pl.schedule?.mode === "CRON" && (
                <div>
                  ...
                  <input
                    className={cx("input mono", styles.cronInput)}
                    value={pl.schedule?.cron || ""}
                    onChange={(e) =>
                      setPl({ ...pl, schedule: { ...(pl.schedule || {}), cron: e.target.value } })
                    }
                    placeholder="0 0 2 * * *"
                  />
                  <div className={styles.cronHint}>
                    ex: «0 0 2 * * *» = chaque jour à 02:00 · «0 0 6 * * MON» = lundi 06:00
                  </div>
                </div>
              )}
              {pl.schedule?.mode === "POLLING" && (
                <div>
                  ...
                  <input
                    className={cx("input mono", styles.intervalInput)}
                    type="number"
                    min="1"
                    ...
                    onChange={(e) =>
                      setPl({
                        ...
                        schedule: {
                          ...(pl.schedule || {}),
                          intervalMinutes: parseInt(e.target.value, 10) || null,
                        },
                      })
                    }
                    placeholder="60"
              ...
              {(!pl.schedule?.mode || pl.schedule?.mode === "MANUAL") && (
                <div className={styles.manualHint}>
                  Les pipelines créés depuis ce template ne s'exécuteront que manuellement (bouton
                  Run / import).
                </div>
              )}
            </div>
